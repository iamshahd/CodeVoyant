...
from src.graph import analyze_repository, CodeAnalyzer
...
Examples:
  ...
  python analyze.py --calls-only --entry-points main.py app.py
    ...
    parser.add_argument(
        '--dependencies-only',
        action='store_true',
        help='Extract only dependency graph'
    )
    
    parser.add_argument(
        '--calls-only',
        action='store_true',
        help='Extract only call graph'
    )
    
    parser.add_argument(
        '--entry-points',
        nargs='+',
        help='Entry point files for call graph analysis'
    )
