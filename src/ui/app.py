...
def display_algorithm_comparison(graph, temp_file_path):
    ...
    st.info("ℹ️ This comparison runs all algorithms independently and is cached per graph upload.")
    ...
    if valid_results:
        ...
        with col1:
            best_modularity = max(valid_results.items(), key=lambda x: x[1].get('modularity', -1))
            st.success(f"🏆 Best modularity: **{best_modularity[0]}** ({best_modularity[1]['modularity']:.4f})")
        
        with col2:
            fastest = min(valid_results.items(), key=lambda x: x[1].get('runtime_seconds', float('inf')))
        ...
        for algo_name, result in sorted(valid_results.items(), key=lambda x: x[1]['runtime_seconds']):
            runtime_ms = result['runtime_seconds'] * 1000
            modularity = result['modularity']
            
            # Create a visual indicator
            speed_emoji = "⚡⚡⚡" if runtime_ms < 10 else "⚡⚡" if runtime_ms < 50 else "⚡"
            quality_emoji = "⭐⭐⭐" if modularity > 0.4 else "⭐⭐" if modularity > 0.3 else "⭐"
    ...
    if uploaded_file is not None:
        ...
        with tab2:
            if enable_communities and communities and analyzer:
                ...
                st.dataframe(
                    metrics_df.sort_values('size', ascending=False),
                    width='stretch'
                )
                ...
                for i, metric in enumerate(top_communities, 1):
                    with st.expander(f"{i}. Community {metric['community_id']} ({metric['size']} nodes)"):
        ...
        with tab3:
            if num_nodes < 1000:  # Only for reasonably sized graphs
                ...
                st.warning("Algorithm comparison is disabled for large graphs (>1000 nodes) to avoid performance issues.")
