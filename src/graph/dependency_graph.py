...
class DependencyGraphExtractor:
        ...
        for root, dirs, files in os.walk(self.repo_path):
            ...
            for file in files:
                if file.endswith(".py"):
                    ...
                    self.graph.add_node(
                        module_name, path=str(file_path.relative_to(self.repo_path))
                    )
        ...
        try:
            ...
            for imported_module in imports:
                ...
                if resolved and resolved in self.module_files:
                    ...
                    self.graph.add_edge(
                        module_name, imported_module, import_type="external"
                    )
    ...
    def _resolve_import(
        self, imported_module: str, current_module: str
    ) -> Optional[str]:
        """
    ...
    def get_statistics(self) -> Dict:
        ...
        """
        local_nodes = [
            n for n, d in self.graph.nodes(data=True) if not d.get("external", False)
        ]
        external_nodes = [
            n for n, d in self.graph.nodes(data=True) if d.get("external", False)
        ]
        ...
        return {
            ...
            "external_dependencies": len(
                [
                    e
                    for e in self.graph.edges(data=True)
                    if e[2].get("import_type") == "external"
                ]
            ),
        }
