...
class CodeAnalyzer:
    ...
    def save_graphs(self, formats: Optional[List[str]] = None) -> Dict[str, str]:
        ...
        if self.dependency_graph:
            ...
            dep_files = self._save_graph(
                self.dependency_graph, "dependency_graph", formats
            )
            output_files.update(dep_files)

        if self.call_graph:
            print("\nSaving call graph...")
            call_files = self._save_graph(self.call_graph, "call_graph", formats)
    ...
    def _save_graph(
        self, graph: nx.DiGraph, graph_type: str, formats: List[str]
    ) -> Dict[str, str]:
        """
        ...
        for fmt in formats:
            if fmt == "json":
                ...
                GraphSerializer.to_json(
                    graph, str(output_path), format_type="node_link"
                )
                ...
                GraphSerializer.to_json(
                    graph, str(output_path), format_type="cytoscape"
                )
...
def analyze_repository(
    ...
    analyzer.analyze(
        extract_dependencies=True, extract_calls=True, entry_points=entry_points
    )
